...
def run_tip_adapter(cfg, cache_keys, cache_values, val_features, val_labels, test_features, test_labels, clip_weights):
    ...
    # Zero-shot CLIP
    # 100 --> temperature scaling factor to prevent vanishing gradient problem
...
def run_tip_adapter_F(cfg, cache_keys, cache_values, val_features, val_labels, test_features, test_labels, clip_weights, clip_model, train_loader_F):
    ...
    for train_idx in range(cfg['train_epoch']):
        ...
        for i, (images, target) in enumerate(tqdm(train_loader_F)):
            ...
            # Get image paths from DataLoader to watch which images are processing.
            # Print image paths only in last epoch
            if train_idx == cfg['train_epoch'] - 1:
                print(f"\nBatch {i+1}:")
                current_batch_paths = train_loader_F.dataset.data_source[i*train_loader_F.batch_size:(i+1)*train_loader_F.batch_size]
                for idx, item in enumerate(current_batch_paths):
                    print(f"{idx+1}. {os.path.basename(item.impath)} (Class: {item.label})")
        ...
        # Eval
        # Used validation set instead of test set in evaluation phase
        adapter.eval()

        affinity = adapter(val_features)
        cache_logits = ((-1) * (beta - beta * affinity)).exp() @ cache_values
        clip_logits_val = 100. * val_features @ clip_weights
        tip_logits_val = clip_logits_val + cache_logits * alpha
        acc = cls_acc(tip_logits_val, val_labels)

        print("**** Tip-Adapter-F's validation accuracy: {:.2f}. ****\n".format(acc))
        if acc > best_acc:
            best_acc = acc
    ...
    print(f"**** After fine-tuning, Tip-Adapter-F's best validation accuracy: {best_acc:.2f}, at epoch: {best_epoch}. ****\n")
    ...
    cache_logits = ((-1) * (best_beta - best_beta * affinity)).exp() @ cache_values
    # Clip logits calculates again for test set 
    clip_logits_test = 100. * test_features @ clip_weights
    tip_logits_test = clip_logits_test + cache_logits * best_alpha
    acc = cls_acc(tip_logits_test, test_labels)
